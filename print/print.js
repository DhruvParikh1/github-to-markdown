/**
 * GitHub to Markdown - Print Script
 * Injected via chrome.scripting.executeScript to print rendered markdown content.
 * Works on any GitHub/GHE page with a .markdown-body or Jupyter notebook.
 */
(function printPage() {
  'use strict';
  ...
  link.rel = 'stylesheet';
  link.href = chrome.runtime.getURL('print/print.css');
  ...
  link.addEventListener('load', async () => {
    ...
    // Use light theme or else text contrast is bad when printed
    document.documentElement.dataset.colorMode = 'light';

    // For all iframes, change 'color_mode=dark' to 'color_mode=light' so that
  ...
  async function waitForMermaidDiagramsToLoad() {
    // Keep track of iframes that have loaded. We aren't able to peer inside the
    // frame due to cross-origin restrictions, so we just wait for the frame to
    // send us a message when it's ready.
  ...
  async function waitForJupyterNotebooksToLoad() {
    // Give jupyter notebooks some time to load before attempting to print.
    // The iframe doesn't send a ready event after content is replaced.
